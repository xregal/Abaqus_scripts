...
jobFileName='standard_elas_10em7_visco'
...
specMat.Elastic(moduli=INSTANTANEOUS, table=((specExx, specNuxy), ))
#specMat.Viscoelastic(domain=FREQUENCY, frequency=PRONY, table=((0.99, 0.99, tau1), ))
#specMat.Viscoelastic(domain=TIME, time=PRONY, table=((0.99, 0.99, tau1), ))	
specMat.Viscoelastic(domain=TIME, time=FREQUENCY_DATA, table=((math.cos(delta), math.sin(delta),math.cos(delta), math.sin(delta), freq0), ))
...
mymodel.DisplacementBC(name='Imposed_disp', 
    ...
    amplitude='Sinwave', fixed=OFF, distributionType=UNIFORM, fieldName='', 
    localCsys=None)	
...
mymodel.HistoryOutputRequest(name='H-Output-1', 
    createStepName='Sonotrode', variables=('ALLAE', 'ALLDC', 'ALLDMD', 
...
for i in range(nsteps-128,nsteps):
